...
function PageElectricite() {
	const { vagueId } = useParams();
	const [electricites, setElectricites] = useState({});
	...
	const [electriciteList, setElectriciteList] = useState(
		JSON.parse(LocalStorage.getItem("electriciteList")) &&
			JSON.parse(LocalStorage.getItem("electriciteList")).length != 0 &&
			JSON.parse(LocalStorage.getItem("electriciteList"))[0].vagueId ==
				vagueId
			? JSON.parse(LocalStorage.getItem("electriciteList"))
			: []
	);
	...
	const getElectricite = () => {
		if (!electriciteList || electriciteList.length == 0) setLoading(true);
		Axios.get(`/api/electricite/all/${vagueId}`)
			.then((response) => {
	...
	const updateElectriciteTitre = (id) => {
		setLoading(true);
		if (newTitre == "") {
			showAlertError("Le champs ne doit pas être vide");
			setLoading(false);
		} else {
			Axios.put(`/api/electricite/${id}`, {
				titre: newTitre,
				_id: id,
			})
				.then((response) => {
	...
	const updateElectricitePrix = (id) => {
		setLoading(true);
		if (newPrix == "") {
			showAlertError("Le champs ne doit pas être vide");
			setLoading(false);
	...
	const onFileUpload = (id) => {
		...
		if (selectedFile == null) {
			...
			Axios.put(`/api/electricite/${id}`, formData, {
	...
	const deleteElectricite = (id) => {
		setLoading(true);
		Axios.delete(`/api/electricite/${id}`)
			.then((response) => {
				getElectricite();
	...
	const setProgressBar = () => {
		...
		progressbar = (electricites.prix * 100) / budget_electricite;
		return progressbar;
	};
	return (
		<div>
			<NavBar
				VAGUEID={vagueId}
				vagueList={vagueList}
				funtionToUpdate={getElectricite}
			/>
...
export default PageElectricite;
