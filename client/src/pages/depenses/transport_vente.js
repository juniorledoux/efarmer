...
function PageTransport_Vente() {
	const { vagueId } = useParams();
	const [transports_Ventes, setTransportsVentes] = useState({});
	...
	const [newTransporteur, setNewTransporteur] = useState("");
	const [newFacture, setNewFacture] = useState("");
	const [transport_venteList, setTransport_venteList] = useState(
		JSON.parse(LocalStorage.getItem("transport_venteList")) &&
			JSON.parse(LocalStorage.getItem("transport_venteList")).length !=
				0 &&
			JSON.parse(LocalStorage.getItem("transport_venteList"))[0]
				.vagueId == vagueId
			? JSON.parse(LocalStorage.getItem("transport_venteList"))
			: []
	);
	...
	const getTransport_vente = () => {
		if (!transport_venteList || transport_venteList.length == 0)
			setLoading(true);
		Axios.get(`/api/transport_vente/all/${vagueId}`)
			.then((response) => {
	...
	const updateTransport_venteDestination = (id) => {
		setLoading(true);
		if (newDestination == "") {
			showAlertError("Le champs ne doit pas être vide");
			setLoading(false);
		} else {
			Axios.put(`/api/transport_vente/${id}`, {
	...
	const updateTransport_ventePrix = (id) => {
		setLoading(true);
		if (newPrix == "") {
			showAlertError("Le champs ne doit pas être vide");
			setLoading(false);
	...
	const updateTransport_venteTransporteur = (id) => {
		setLoading(true);
		if (newTransporteur == "") {
			showAlertError("Le champs ne doit pas être vide");
			setLoading(false);
	...
	const onFileUpload = (id) => {
		...
		//axios.post("/api/transport_vente", formData);

		if (selectedFile == null) {
			showAlertError("Le champs ne doit pas être vide");
			setLoading(false);
		} else {
			Axios.put(`/api/transport_vente/${id}`, formData, {
	...
	const deleteTransport_vente = (id) => {
		setLoading(true);
		Axios.delete(`/api/transport_vente/${id}`)
			.then((response) => {
				getTransport_vente();
	...
	const setProgressBar = () => {
		...
		progressbar = (transports_Ventes.prix * 100) / budget_transport_vente;
		return progressbar;
	};
	return (
		<div>
			<NavBar
				VAGUEID={vagueId}
				vagueList={vagueList}
				funtionToUpdate={getTransport_vente}
			/>
																		...
																		>
																			<div className="form-group text-center">
																				...
																				<button
																					...
																					onClick={() => {
																						updateTransport_venteDestination(
																							val._id
																						);
																					}}
																				>
																		...
																		>
																			<div className="form-group text-center">
																				...
																				<button
																					className="btn btn-outline-success mt-2 btn-fw"
																					onClick={() => {
																						updateTransport_venteTransporteur(
																							val._id
																						);
																					}}
																				>
...
export default PageTransport_Vente;
