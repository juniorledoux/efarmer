...
function PageMain_Doeuvre() {
	const { vagueId } = useParams();
	const [mains_Doeuvre, setMainsDoeuvre] = useState({});
	...
	const [main_deouvreList, setMain_DeouvreList] = useState(
		JSON.parse(LocalStorage.getItem("main_deouvreList")) &&
			JSON.parse(LocalStorage.getItem("main_deouvreList")).length != 0 &&
			JSON.parse(LocalStorage.getItem("main_deouvreList"))[0].vagueId ==
				vagueId
			? JSON.parse(LocalStorage.getItem("main_deouvreList"))
			: []
	);
	...
	const getMain_deouvre = () => {
		if (!main_deouvreList || main_deouvreList.length == 0) setLoading(true);
		Axios.get(`/api/main_doeuvre/all/${vagueId}`)
			.then((response) => {
	...
	const updateMain_deouvreTitre = (id) => {
		setLoading(true);
		if (newTitre == "") {
			showAlertError("Le champs ne doit pas être vide");
			setLoading(false);
		} else {
			Axios.put(`/api/main_doeuvre/${id}`, {
				titre: newTitre,
				_id: id,
			})
				.then((response) => {
	...
	const updateMain_deouvrePrix = (id) => {
		setLoading(true);
		if (newPrix == "") {
			showAlertError("Le champs ne doit pas être vide");
			setLoading(false);
	...
	const onFileUpload = (id) => {
		...
		if (selectedFile == null) {
			...
			Axios.put(`/api/main_doeuvre/${id}`, formData, {
	...
	const deleteMain_deouvre = (id) => {
		setLoading(true);
		Axios.delete(`/api/main_doeuvre/${id}`)
			.then((response) => {
				getMain_deouvre();
	...
	const setProgressBar = () => {
		...
		progressbar = (mains_Doeuvre.prix * 100) / budget_main_doeuvre;
		return progressbar;
	};
	return (
		<div>
			<NavBar
				VAGUEID={vagueId}
				vagueList={vagueList}
				funtionToUpdate={getMain_deouvre}
			/>
...
export default PageMain_Doeuvre;
