...
function PageAmenage() {
	...
	const [amenagementList, setAmenagementList] = useState(
		JSON.parse(LocalStorage.getItem("amenagementList")) &&
			JSON.parse(LocalStorage.getItem("amenagementList")).length != 0 &&
			JSON.parse(LocalStorage.getItem("amenagementList"))[0].vagueId ==
				vagueId
			? JSON.parse(LocalStorage.getItem("amenagementList"))
			: []
	);
	...
	const getAmenage = () => {
		if (!amenagementList || amenagementList.length == 0) setLoading(true);
		Axios.get(`/api/amenagement/all/${vagueId}`)
			.then((response) => {
	...
	const updateAmenageTitre = (id) => {
		setLoading(true);
		if (newTitre == "") {
			showAlertError("Le champs ne doit pas être vide");
			setLoading(false);
		} else {
			Axios.put(`/api/amenagement/${id}`, {
				titre: newTitre,
				_id: id,
			})
				.then((response) => {
	...
	const onFileUpload = (id) => {
		...
		if (selectedFile == null) {
			...
			Axios.put(`/api/amenagement/${id}`, formData, {
	...
	const deleteAmenage = (id) => {
		setLoading(true);
		Axios.delete(`/api/amenagement/${id}`)
			.then((response) => {
				getAmenage();
