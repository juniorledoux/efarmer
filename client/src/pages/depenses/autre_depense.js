...
function PageAutreDepense() {
	const { vagueId } = useParams();
	const [autres_depenses, setAutresDepenses] = useState({});
	...
	const [autreDepenseList, setAutreDepenseList] = useState(
		JSON.parse(LocalStorage.getItem("autreDepenseList")) &&
			JSON.parse(LocalStorage.getItem("autreDepenseList")).length != 0 &&
			JSON.parse(LocalStorage.getItem("autreDepenseList"))[0].vagueId ==
				vagueId
			? JSON.parse(LocalStorage.getItem("autreDepenseList"))
			: []
	);
	...
	const getAutreDepenses = () => {
		if (!autreDepenseList || autreDepenseList.length == 0) setLoading(true);
		Axios.get(`/api/autre_depense/all/${vagueId}`)
			.then((response) => {
	...
	const updateDepenseTitre = (id) => {
		setLoading(true);
		if (newTitre == "") {
			showAlertError("Le champs ne doit pas être vide");
			setLoading(false);
		} else {
			Axios.put(`/api/autre_depense/${id}`, {
				titre: newTitre,
				_id: id,
			})
				.then((response) => {
	...
	const onFileUpload = (id) => {
		...
		if (selectedFile == null) {
			...
			Axios.put(`/api/autre_depense/${id}`, formData, {
	...
	const deleteDepense = (id) => {
		setLoading(true);
		Axios.delete(`/api/autre_depense/${id}`)
			.then((response) => {
	...
	const setProgressBar = () => {
		...
		progressbar = (autres_depenses.prix * 100) / budget_autres_depenses;
		return progressbar;
	};
	return (
		<div>
			<NavBar
				VAGUEID={vagueId}
				vagueList={vagueList}
				funtionToUpdate={getAutreDepenses}
			/>
...
export default PageAutreDepense;
